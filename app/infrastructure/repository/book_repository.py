...
class BookRepository(IBookRepository):
        ...
        with get_session() as session:
            ...
            if category:
                ...
                query = query.join(Category).filter(
                    func.lower(Category.name) == cat_name
                )
    ...
    def get_books_by_price(
        ...
        max_price: Decimal = Decimal("Infinity"),
    ) -> tuple[list[DomainBook], int]:
        with get_session() as session:
            ...
            offset = (page - 1) * per_page
            books_orm = filtered_query.offset(offset).limit(per_page).all()
    ...
    def get_overview_stats(self) -> dict[str, Any]:
        with get_session() as session:
            ...
            avg_price = (
                round(float(avg_price_dec), 2) if avg_price_dec is not None else None
            )

            rows = (
                session.query(Book.rating, func.count(Book.id))

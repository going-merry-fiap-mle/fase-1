...
if [ -n "${DYNO:-}" ]; then
  echo "[INFO] Heroku detected (DYNO=$DYNO)"
  echo "[INFO] Datadog APM disabled - using log drain for monitoring"
  
  if [ -z "${DD_LOGS_INJECTION:-}" ]; then
    export DD_LOGS_INJECTION="true"
    echo "[INFO] DD_LOGS_INJECTION enabled for log correlation"
  fi
fi

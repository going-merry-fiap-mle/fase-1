...
cleanup() {
    echo "[INFO] Received shutdown signal, cleaning up gracefully..."

    if [ -n "$FLASK_PID" ] && kill -0 $FLASK_PID 2>/dev/null; then
        echo "[INFO] Stopping Flask application (PID: $FLASK_PID)..."
        kill -TERM $FLASK_PID 2>/dev/null || true
        wait $FLASK_PID 2>/dev/null || true
    fi

    if [ -n "$TRACE_AGENT_PID" ] && kill -0 $TRACE_AGENT_PID 2>/dev/null; then
        echo "[INFO] Stopping Datadog Trace Agent (PID: $TRACE_AGENT_PID)..."
        kill -TERM $TRACE_AGENT_PID 2>/dev/null || true
        wait $TRACE_AGENT_PID 2>/dev/null || true
    fi

    if [ -n "$XVFB_PID" ] && kill -0 $XVFB_PID 2>/dev/null; then
        echo "[INFO] Stopping Xvfb (PID: $XVFB_PID)..."
...
if kill -0 $XVFB_PID 2>/dev/null; then
  ...
  echo "[WARN] Xvfb may have failed to start"
fi

echo "[INFO] Starting Flask application..."
/app/start-backend.sh &
FLASK_PID=$!
echo "[INFO] Flask application PID: $FLASK_PID"
